...
Mode: brainstorm
Models: gpt-5.4 (11.52s), grok-4.20-experimental-beta-0304-reasoning (19.83s), dashscope/qwen3.5-plus (21.02s), claude-opus-4-6 (25.89s), gemini-3.1-pro-preview (62.04s), redpill/moonshotai/kimi-k2.5 (162.84s)
...
partner models (Gemini, Claude, GPT, Grok, Qwen) regardless of vault_mode.
...
encrypted RedPill models (GLM-5, DeepSeek V3.2/R1, Kimi K2.5, GLM-4.7
...
- **Expected behavior:**
  ...
  2. No data sent to OpenAI, Anthropic, Google, or xAI directly
  3. All inference via RedPill TEE API or self-hosted vLLM
  4. Embeddings computed locally (bge-m3, no OpenAI embedding API)
- **Mode:** Vault
- **Priority:** P3
- **Status:** Implemented
...
- **Expected behavior:**
  1. Full model roster available (GPT-5.4, Claude Opus, Gemini Pro, Grok, etc.)
  2. Queries sent to provider APIs (OpenAI, Anthropic, Google Vertex, xAI)
...
### grok-4.20-experimental-beta-0304-reasoning (19.83s)
**Review Summary**
...
[INFO] Security fix addresses zero-leakage guarantee — J4/J5 fix prevents vault mode queries from leaking to external APIs (OpenAI, Anthropic, Google, xAI) when selected_models is empty. RedPill TEE-encrypted models (GLM-5, DeepSeek V3.2/R1, Kimi K2.5, GLM-4.7 Flash) now used for all tier fallbacks in vault mode.
...
[INFO] `streaming_handler.py:588`: Vault balanced tier uses only 2 models (`REDPILL_GLM5_LLM, REDPILL_DSV3_LLM`) while partner balanced uses 3 (`QUALITY_LLM, CLAUDE_LLM, OPENAI_LLM`). This is a reasonable asymmetry given the smaller vault model roster, but the log message says "using {len} models" which will correctly reflect this — just noting the UX difference (vault balanced gives 2 perspectives, partner gives 3).

[WARNING] `streaming_handler.py:595-597`: The vault expert tier defaults to `REDPILL_GLM5_LLM` which is also the first model in the vault deep tier list. If GLM-5 has an outage, both expert and deep tier lose their primary model. Partner mode has `QUALITY_LLM` (Gemini) for expert and a diverse set for deep. This isn't a regression (partner mode had the same pattern with `QUALITY_LLM`), but the vault mode concentration risk is higher since all models route through a single provider (RedPill). Concrete failure mode: RedPill API outage takes down all vault tiers simultaneously. This is inherent to the vault architecture though, not introduced by this diff.
...
*   **The "Block" Threshold:** There is a sharp divide on whether this PR is ready. **Grok-4.20** and **gpt-5.4** issue a `[BLOCK]` based on the lack of end-to-end (E2E) testing for the security regression. **Claude-opus-4-6** and **qwen3.5-plus** acknowledge the testing gap but argue the code change is sufficiently simple and correct to proceed without a block.
...
*   **The "Balanced" Tier Cardinality:** There is a split on the reduction of models in the "balanced" tier (from 3 to 2). **Grok** and **Qwen** view this as an acceptable, intentional trade-off. **Kimi-k2.5** suggests it may be an oversight that breaks tier parity and should be addressed.

### Unique/Creative Insights
*   **The "Malicious Client" Vulnerability:** **Gemini-3.1** identified a `[PRE-EXISTING]` security gap: even if the fallback logic is fixed, the system does not currently validate that *user-provided* `selected_models` are vault-compliant. This suggests the current fix is a "partial" security solution.
*   **Concentration Risk:** **Claude-opus-4-6** highlighted that by routing all vault tiers through RedPill, the system has introduced a single point of failure (RedPill API outage) that didn't exist in the same way for partner modes.
*   **Handover Hygiene:** **Kimi-k2.5** flagged a violation of Ground Rule #3 (Blast Radius), noting that the inclusion of documentation updates in the same commit pushed the diff over the 300-line limit.
...
2.  **Address the Pre-existing Gap:** Before merging, acknowledge the `[PRE-EXISTING]` vulnerability identified by Gemini regarding user-supplied `selected_models`. Even if you do not fix it in this PR, adding a `TODO` or a validation filter is highly recommended given the context of REQ-013.
